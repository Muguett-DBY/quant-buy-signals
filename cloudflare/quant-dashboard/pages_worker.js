...
function renderVerdict(r){
  ...
  if(triggered.length){
    ...
    note="满足前置条件但还需确认："+names(conditional)+"。点击对应类型查看附加条件"+(hasPositionConfirmation?"与仓位确认要求":"")+"。";
...
async function load(){
  ...
  try{
    ...
    const cards=[["纳入分析公司",s.company_count||data.length],["至少一种已触发",s.triggered_company_count||0],["待满足附加条件",s.conditional_company_count||0],["其中待确认仓位",actionConfirmationCount],["有客观资料缺口",evidenceGapCompanyCount],["缺口可能改变结论",decisionRelevantGapCount],["七类触发次数",Object.values(s.type_coverage||{}).reduce((n,v)=>n+Number(v.triggered||0),0)]];
    ...
    $("meta").textContent="数据日期："+(m.market_as_of||"—")+" · 更新时间："+formatBeijing(m.data_timestamp_utc)+" · 来源版本："+(sourceVersion||"—")+" · 量化口径："+METHODOLOGY_LABEL+" · "+analysisScopeText(scope,s.company_count||data.length);
