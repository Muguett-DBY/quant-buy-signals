...
const RULE_SOURCES=Object.freeze([
  ...
  Object.freeze({id:"patch5",filename:"补丁5.md",sha256:"8E1C5114BE74254D686AC2B65EC7B3563E09F6C3B3F9A82B43E4D60A84CA42A4"}),
  Object.freeze({id:"patch6",filename:"补丁6· 公司三属性分类与三维度量化打分机制.md",sha256:"DFADE9961A182BFFF67F95E2F8D55FD637CF8A15CEDD44C12300B4F9C4C1549B"}),
  Object.freeze({id:"patch7",filename:"补丁7· 长期投资者的买卖总闸门（七种买入情况+量化打分+卖出闸门）.md",sha256:"69B6BBEAA44755B9935518C665BC1AC0CAC5C473AABA5B106BDF0F9FC88BEB6D"}),
  Object.freeze({id:"subsequent_addenda",filename:"后续附加补丁们.md",sha256:"0DEA9125BBE2039ACF741AC997E62B53C49B6E3DC32E7D956ED96F9D7054B64F"}),
]);
const RULE_SOURCE_CONTRACT=Object.freeze({
  hash_algorithm:"SHA-256",
  source_count:RULE_SOURCES.length,
  sources:RULE_SOURCES,
  precedence:Object.freeze(["current_independent_patch6_and_patch7","independent_templates_and_patches_1_to_5","subsequent_addenda_unique_content","historical_aggregations"]),
...
const SHARED_FUTURE_FCF_THRESHOLD="补丁7共同前提：未来最终能创造可观自由现金流；证据完整且通过才可触发，明确失败否决，资料缺失或不完整为待补";
const PATCH7_TOTAL_GATE=Object.freeze({
  version:"2026-08-04",
  decision_domain:"new_buy_or_add",
  evaluation_order:"七类各自评分与前置门槛完成后，再执行跨类型总闸门，最后发布状态",
  rules:Object.freeze([
    Object.freeze({when:"only_type1",types:Object.freeze(["type1"]),requirement:"无价值陷阱，且不踩衰落产业、长期经营走下坡路两条红线"}),
    Object.freeze({when:"only_or_combination_of_type2_type3_type4_with_optional_type7",types:Object.freeze(["type2","type3","type4","type7"]),requirement:"股价不得处于泡沫区；泡沫线为乐观值×120%"}),
    Object.freeze({when:"only_type7",types:Object.freeze(["type7"]),requirement:"股价必须合理或低估"}),
    Object.freeze({when:"every_still_triggered_type",types:Object.freeze(["type1","type2","type3","type4","type5","type6","type7"]),requirement:"必须有可核验的未来自由现金流前提；明确失败则否决，资料不足则待补"}),
  ]),
  outside_extra_gate:"除共同未来自由现金流前提外，包含Type 1的组合、Type 5、Type 6及其他组合只按各类型自身规则处理，不额外增加价格或红线组合门槛",
});
const TYPE5_APPENDIX=Object.freeze({
  applicability:"5a≥7",
  trigger:"5b至5e证据完整且五项加权总分≥7",
  weights:Object.freeze({"5a":35,"5b":25,"5c":20,"5d":10,"5e":10}),
  no_extra_gate:"5c只按20%权重进入总分，不另设5分门槛，也不存在3分否决线",
  below_threshold:"5a<7表示情况五不适用；5a≥7但总分<7表示适用但当前不是买点",
});
const SELL_DOMAIN=Object.freeze({
  implemented:false,
  decision_domain:"existing_holding_sell_or_trim",
  default_state:"持有",
  core_rule:"买入不触发≠卖出",
  reason:"卖出判断需要持仓成本、原始买入逻辑、实际仓位、组合暴露和可比替代标的；全市场公开数据不足以替代这些投资者输入",
  hard_triggers:Object.freeze(["核心逻辑证伪","价格进入卖区或泡沫线","出现同质地且显著更便宜的替代标的","实际仓位超过配置上限"]),
  soft_permission:"短期获利丰厚时可极小减仓，非必须动作",
  public_output:"当前网站不生成卖出信号",
});
const METHODOLOGY={
  type1:{
    summary:"用经过校验的估值结果判断现价是否进入买入区，同时排除价值陷阱并检查现金回报与回归动力。",
    applicability:"适用于能够形成可核验估值基准的公司；金融股使用专属市净率与监管口径。",
    trigger:"证据完整、加权总分至少7分，实际处于买入区，并且价格深度与价值陷阱检查不触发硬性否决。",
    key_thresholds:["加权总分≥7","实际处于对应估值模型买入区","1a≤2直接否决","价值陷阱证据完整且1b≤3直接否决","补丁7仅情况一红线依赖行业专属中位增长率及样本量，并需最近连续4年收入；任一证据缺失时资料待补，不默认通过",SHARED_FUTURE_FCF_THRESHOLD],
    dimensions:{
  ...
  type2:{
    ...
    key_thresholds:["加权总分≥7","(2a+2b)/2>4","2c>3","通常2d≥5；若两热平均≥7且2c≥7，则2d为4至5也可通过",SHARED_FUTURE_FCF_THRESHOLD],
    dimensions:{
      ...
      "2d":{weight:20,meaning:"当前估值在公司自身历史中是否合理。",data:"公司自身近五年PE和PB分布、当前PE/PB与历史分位；上市不足五年的公司用其全部可查历史并在说明中标明实际年数。",direction:"处于自身历史较低分位时分数更高；不能用同行估值替代自身历史。"}
    }
  },
  type3:{
    ...
    key_thresholds:["非金融公司","长期趋势增速≥10%","加权总分≥7","可追溯完整证据下3a≤3或3d≤3直接否决","完整泡沫证据下3e≤3时总分封顶4.9",SHARED_FUTURE_FCF_THRESHOLD],
    dimensions:{
  ...
  type4:{
    ...
    key_thresholds:["非金融公司","加权总分≥7","护城河证据完整且4c≤3直接否决","产业泡沫和股价透支证据完整且4e≤3、4f≤3时直接否决",SHARED_FUTURE_FCF_THRESHOLD],
    dimensions:{
  ...
  type5:{
    ...
    key_thresholds:["5a≥7才适用","5b至5e证据完整","五项加权总分≥7","5c只按20%计权，不另设独立触发线或否决线",SHARED_FUTURE_FCF_THRESHOLD],
    dimensions:{
  ...
  type6:{
    ...
    key_thresholds:["高景气技术型要求行业增速≥20%、公司趋势增速≥30%、市值≤300亿元","平稳产业反转型市值≤100亿元且6d≥5","6a至6d至少两项≥5","6b或6c的间接代理最多4分，且不计入核心项达到5分","加权总分≥7","确认单票≤5%、同类组合≤15%",SHARED_FUTURE_FCF_THRESHOLD],
    dimensions:{
  ...
  type7:{
    ...
    key_thresholds:["非金融公司","7a、7b、7c证据完整且算术平均严格>7.000","强周期7a或7b<5直接否决","强科技7a、7b、7c分别≥7","未来自由现金流前提与类别路径完整","弱周期价格位置分≥3；强科技近五年PB分位≤20%；强周期PB≤1.20且近五年PB分位≤20%",SHARED_FUTURE_FCF_THRESHOLD],
    dimensions:{
...
<html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><title>模型实际触发候选｜七类量化买入看板</title>
...
.verdict-tag.buy{background:#16a34a}.verdict-tag.watch{background:#d97706}.verdict-tag.avoid{background:#dc2626}.verdict-tag.gap{background:#7c3aed}.verdict-tag.neutral{background:#64748b}.verdict-tag.blocked{background:#9a3412}
...
.dimension-summary{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.decision-scope{border-color:#bfdbfe;background:#eff6ff}.decision-scope h2{color:#1e3a8a}.decision-scope p{margin:5px 0}.decision-scope details{margin-top:8px}.decision-scope summary{cursor:pointer;font-weight:700;color:#1e40af}.decision-scope ol{margin:7px 0 0;padding-left:22px}
...
</style></head><body><header id="pageHeader"><h1>模型实际触发候选</h1><p>沪深市场七类量化买入看板 · 默认只列真实触发，不把诊断高分当成信号</p></header><main class="wrap" id="mainContent"><section class="cards" id="cards"></section><section class="panel decision-scope"><h2 class="panel-title">当前网站决策范围</h2><p><strong>默认仅展示至少一个类型状态为已触发的公司。</strong>只有真实 triggered / buy_types 才进入默认候选；conditional/pending 均不是买入信号。</p><p><strong>使用边界：</strong>这是按公开数据生成的研究候选清单，不是收益保证或个性化投资建议；请打开公司核对硬条件、资料缺口和估值依据后再决策。</p><p>首页前四项按“实际触发 → 待行动确认/附加条件 → 资料待补 → 其余当前不买”的顺序互斥归类；后三类都不是买入信号。</p><p>待行动/附加确认、资料待补、观察、未触发均不是买入信号；七类诊断最高分只用于研究排序，不能替代实际触发状态。</p><p><strong>买入不触发≠卖出。</strong>本页只覆盖新买入或加仓域；卖出闸门需要持仓成本、原始买入逻辑和实际组合仓位，当前网站不生成卖出信号。</p><details><summary>补丁7跨类型总闸门</summary><ol><li>仅情况一满足：排除价值陷阱、衰落产业和长期经营走下坡路。</li><li>仅情况二、三、四或其组合满足，或它们与情况七组合满足：价格不得达到乐观值×120%的泡沫线。</li><li>仅情况七满足：价格必须合理或低估。</li><li>所有仍满足条件的类型：必须通过可核验的未来自由现金流前提；明确失败则否决，资料不足则待补。</li></ol></details></section><section class="panel"><div class="meta" id="meta">正在读取数据…</div><div class="notice" id="notice" hidden></div></section><section class="panel"><h2 class="panel-title">七类实际触发、待确认与证据完整度分布</h2><p class="coverage-help">“已触发”才是实际买入候选。“结论相关”表示任何待补项仍可能改变当前结论，应优先核验；“代理待核验”表示已有量化估算、仍缺原始资料确认，并非抓取失败；“缺直接/结构化证据”还可能来自历史不足或公司未披露，不等同于抓取失败，也不保证能够补齐；“结论已锁定”表示即使仍有未核验维度也不会改变当前不买结论。结论相关的直接证据与代理核验可以在同一家公司重叠，不能相加；三类均不是买入信号。</p><div class="coverage" id="coverage"></div></section><section class="panel"><div class="filters"><label>代码/名称<input id="q" autocomplete="off" inputmode="search" placeholder="例如 600519 或 贵州茅台"></label><label>市场<select id="market"><option value="">全部市场</option><option value="SH">沪市</option><option value="SZ">深市</option></select></label><label>买入类型<select id="type"><option value="">全部七类</option></select></label><label id="statusLabel">当前类型状态<select id="status"><option value="triggered">模型实际触发候选</option><option value="">全部适用状态</option></select><small class="filter-hint" id="statusHint" aria-live="polite"></small></label><label>行业<select id="industry"><option value="">全部行业</option></select></label><label>排序<select id="sort"><option value="score">真实触发优先/实际主类型分数</option><option value="name">名称</option><option value="code">代码</option></select></label></div></section><section class="panel" id="resultsPanel"><div class="meta" id="resultMeta" tabindex="-1" aria-live="polite"></div><div class="table-wrap"><table class="table"><thead><tr><th>代码</th><th>名称</th><th>市场</th><th>行业</th><th id="scoreHeading">实际主类型触发分</th><th id="typeHeading">主要买入情况</th><th id="statusHeading">七类状态</th></tr></thead><tbody id="rows"></tbody></table></div><div class="pager"><span id="pageInfo" aria-live="polite"></span><span><button id="prev">上一页</button> <button id="next">下一页</button></span></div></section></main><div class="drawer" id="drawer" role="dialog" aria-modal="true" aria-labelledby="detailTitle"><article class="drawer-card" id="drawerCard"><div class="drawer-head"><div><h2 id="detailTitle"></h2><div class="meta" id="detailMeta"></div></div><button id="close">关闭</button></div><div class="detail-rows" id="detailRows"></div></article></div><script>
...
const TYPE_PRIORITY=["type1","type2","type5","type3","type4","type6","type7"];
const STATUS_NAMES={triggered:"已触发",conditional:"待确认",observe:"观察",insufficient_evidence:"资料不足",evidence_gap:"存在未核验维度",decision_relevant:"结论相关资料待补",source_gap:"直接/结构化资料待补",proxy_verification:"量化代理待原始资料核验",bounded:"未完全核验但结论已锁定",vetoed:"不符合硬条件",not_triggered:"未触发",not_applicable:"不适用",blocked:"市场状态阻断"};
...
const DECISION_BASIS_NAMES={full_evidence:"证据完整",scope_exclusion:"超出适用范围",confirmed_veto:"已有证据足以否决",conservative_upper_bound:"即使缺失项取最高分也不会触发",action_condition:"仓位输入仍可能改变结论",market_block:"交易状态阻断",unresolved_missing_evidence:"缺失资料仍可能改变结论"};
...
const EXCLUSION_REASON_NAMES={delisting:"退市整理",special_treatment:"ST/特别处理",suspended_or_no_trade:"停牌或无成交",invalid_market_cap:"市值数据无效",incomplete_financial_evidence:"财务证据不完整",stale_or_incomplete_current_financials:"最新财务数据陈旧或不完整",missing_comparative_interim:"缺少同期中报",unclassified_industry:"行业未分类",not_eligible:"其他未纳入"};
const $=id=>document.getElementById(id);const pageSize=50,MAX_DETAIL_PREFETCHES=2;let data=[],page=0,generationId="",marketAsOf="",sourceVersion="",dataMethodologyVersion="",dimensionScoresAvailable=false,dimensionEstimatesAvailable=false,type7MethodDetailAvailable=false,returnFocus=null,composing=false,renderFrame=0,activeDetailRequest=0,activeDetailCode="",detailAbort=null,activeDetailPrefetches=0,idlePrefetchScheduled=false;const detailCache=new Map(),detailPrefetching=new Map();
...
function positionConfirmationRequired(typeKey,value){const decision=value?.decision||{},missing=Array.isArray(decision.missing_dimensions)?decision.missing_dimensions:[];return typeKey==="type6"&&decision.decision_basis==="action_condition"&&decision.potentially_triggerable===true&&missing.includes("6e")}
function analysisScope(manifest,fallbackEligible){const validation=manifest?.provenance?.snapshot_validation||{},market=Number(validation.analysis_market_quotes),eligible=Number(validation.eligible_companies??fallbackEligible);if(!(market>0&&eligible>=0&&eligible<=market))return null;const reasonCounts={};for(const reason of Object.values(validation.analysis_exclusions||{})){if(reason==="unsupported_market")continue;reasonCounts[reason]=(reasonCounts[reason]||0)+1}return{market,eligible,excluded:market-eligible,coverage:Number(validation.analysis_eligible_coverage??eligible/market),reasons:Object.entries(reasonCounts).sort((a,b)=>b[1]-a[1])}}
function analysisScopeText(scope,fallbackEligible){if(!scope)return"纳入分析公司："+Number(fallbackEligible||0).toLocaleString()+" 家";const reasons=scope.reasons.map(([reason,count])=>(EXCLUSION_REASON_NAMES[reason]||"其他原因")+" "+count.toLocaleString()+" 家").join("、");return"分析范围：沪深市场 "+scope.market.toLocaleString()+" 家，纳入 "+scope.eligible.toLocaleString()+" 家（"+(scope.coverage*100).toFixed(2)+"%），未纳入 "+scope.excluded.toLocaleString()+" 家"+(reasons?"；原因："+reasons:"")}
function decisionCategoryCounts(rows,summary={}){
  const companyCount=rows.length;
  const triggeredCompanyCount=rows.filter(row=>actualBuyTypes(row).length>0).length;
  const conditionalOnlyCompanyCount=rows.filter(row=>actualBuyTypes(row).length===0&&Object.values(row.types||{}).some(value=>value?.status==="conditional")).length;
  const declaredVisible=Number(summary.visible_candidate_company_count),declaredPending=Number(summary.pending_company_count);
  const visibleCandidateCompanyCount=Number.isFinite(declaredVisible)?declaredVisible:Math.min(companyCount,triggeredCompanyCount+conditionalOnlyCompanyCount+(Number.isFinite(declaredPending)?declaredPending:0));
  const pendingOnlyCompanyCount=Math.max(0,visibleCandidateCompanyCount-triggeredCompanyCount-conditionalOnlyCompanyCount);
  const currentNoBuyCount=Math.max(0,companyCount-triggeredCompanyCount-conditionalOnlyCompanyCount-pendingOnlyCompanyCount);
  return{companyCount,triggeredCompanyCount,conditionalOnlyCompanyCount,pendingOnlyCompanyCount,currentNoBuyCount};
}
...
function typeStatusMatches(typeState,status){if(status==="evidence_gap")return typeState?.has_evidence_gap===true;if(status==="decision_relevant")return typeState?.decision_relevant===true;if(status==="source_gap")return typeState?.source_gap===true;if(status==="proxy_verification")return typeState?.proxy_verification===true;if(status==="bounded")return typeState?.bounded===true;return typeState?.status===status}
function rowMatches(r,{q,m,t,s,i}){const typeState=t?(r.types?.[t]||null):null,selectedTypeMatches=s==="triggered"?actualBuyTypes(r).includes(t):typeStatusMatches(typeState,s);const typeMatches=!t||(typeState&&(q||s?(!s||selectedTypeMatches):typeState.status!=="not_applicable"));const statusMatches=!s||Boolean(t)||(s==="triggered"?actualBuyTypes(r).length>0:Object.values(r.types||{}).some(value=>typeStatusMatches(value,s)));return Boolean((!q||r._search.includes(q))&&(!m||r._market===m)&&(!i||r.industry===i)&&typeMatches&&statusMatches)}
function filtered(){const q=$("q").value.trim().toLowerCase(),m=$("market").value,t=$("type").value,s=q?"":$("status").value,i=$("industry").value;const out=data.filter(r=>rowMatches(r,{q,m,t,s,i}));out.sort((a,b)=>$("sort").value==="name"?String(a.name).localeCompare(String(b.name),"zh-CN"):$("sort").value==="code"?String(a.code).localeCompare(String(b.code)):compareRowsByScore(a,b,t));return out}
...
function coverageEvidenceStats(record,indexRecord={}){return{evidenceMissing:Number(indexRecord?.evidence_missing??record?.insufficient_evidence??0),sourceMissing:Number(indexRecord?.source_missing??0),proxyVerification:Number(indexRecord?.proxy_verification??0),decisionRelevant:Number(indexRecord?.decision_relevant??indexRecord?.decision_unresolved??0),sourceDecisionRelevant:Number(indexRecord?.source_decision_relevant??0),proxyDecisionRelevant:Number(indexRecord?.proxy_decision_relevant??0),bounded:Number(indexRecord?.bounded??0),potentiallyTriggerable:Number(indexRecord?.potentially_triggerable??0),actionConfirmation:Number(indexRecord?.action_confirmation??0)}}
...
function renderCoverage(summary,indexCoverage={}){
  ...
  const nodes=Object.entries(TYPE_NAMES).map(([key,name])=>{
    ...
    const conditionalLabel=conditionalCoverageLabel(key,conditional,evidence.actionConfirmation),qualityLabel=key==="type7"?"质量已达标 "+qualityCertified.toLocaleString()+" 家":"",evidenceLabel="未完全核验 "+evidence.evidenceMissing.toLocaleString()+" 家（结论相关并集 "+evidence.decisionRelevant.toLocaleString()+"；其中直接/结构化资料 "+evidence.sourceDecisionRelevant.toLocaleString()+"、代理待核验 "+evidence.proxyDecisionRelevant.toLocaleString()+"，两者可重叠；全量结论已锁定 "+evidence.bounded.toLocaleString()+"；补齐后仍可能触发 "+evidence.potentiallyTriggerable.toLocaleString()+"）";
    const preferredStatus=triggered>0?"triggered":conditional>0?"conditional":evidence.decisionRelevant>0?"decision_relevant":evidence.proxyVerification>0?"proxy_verification":evidence.sourceMissing>0?"source_gap":evidence.evidenceMissing>0?"evidence_gap":"";
...
function render(){syncSearchStatus();const t=$("type").value,out=filtered(),pages=Math.max(1,Math.ceil(out.length/pageSize));page=Math.min(page,pages-1);const slice=out.slice(page*pageSize,(page+1)*pageSize);$("scoreHeading").textContent=t?TYPE_NAMES[t]+"分数或范围":"实际主类型触发分";$("typeHeading").textContent=t?"当前类型状态":"主要买入情况";$("statusHeading").textContent=t?"查看明细":"七类状态";const fragment=document.createDocumentFragment();for(const r of slice){const tr=document.createElement("tr");tr.tabIndex=0;tr.dataset.code=r.code;tr.setAttribute("aria-label","查看 "+r.name+" "+r.code+" 的七类明细");const primaryType=primaryTriggeredType(r),primaryScore=primaryTriggeredScore(r),displayedScore=t?(scoreLabel(t,r.types?.[t])||"—"):(primaryScore===null?"—":primaryScore.toFixed(1));for(const value of [r.code,r.name,r._market==="SH"?"沪市":"深市",r.industry||"—",displayedScore]){const td=document.createElement("td");td.textContent=value;tr.append(td)}const td=document.createElement("td");if(t){td.append(badge(r.types?.[t]?.status||"invalid","",t))}else{td.textContent=primaryType?TYPE_NAMES[primaryType]:"无触发（不买）"}tr.append(td);const statuses=document.createElement("td");if(t){statuses.textContent="点击查看完整依据"}else{for(const k of Object.keys(TYPE_NAMES)){const v=r.types?.[k];if(v)statuses.append(badge(v.status,k.slice(4)+" "+(STATUS_NAMES[v.status]||"异常"),k))}}tr.append(statuses);fragment.append(tr)}$("rows").replaceChildren(fragment);const hasQuery=$("q").value.trim().length>0;if(out.length)$("resultMeta").textContent="筛选后 "+out.length.toLocaleString()+" 家，当前显示 "+slice.length+" 家；"+(hasQuery?"代码/名称搜索已跨全部状态。":"")+"点击公司查看七类完整明细。";else if(hasQuery)$("resultMeta").textContent="已跨全部状态搜索，但没有找到匹配该代码或名称的公司。";else if(t&&$("status").value==="triggered")$("resultMeta").textContent=TYPE_NAMES[t]+"当前确实为 0 家命中；可切换状态查看未触发原因。";else $("resultMeta").textContent="当前筛选条件没有公司，请调整市场、行业或状态。";$("pageInfo").textContent="第 "+(page+1)+" / "+pages+" 页";$("prev").disabled=page===0;$("next").disabled=page>=pages-1}
...
function publicReasonText(value){
  ...
  for(const [legacy,readable] of [["第1模板","长期质量与回报评分"],["第5模板","产业质量与估值评分"],["补丁5安全边际","安全边际"],["补丁5","商业质量与安全边际评分"],["补丁6","公司属性分类与三维评分"],["补丁7","七类买入与总闸门规则"],["模板25","金融公司估值方法"],["投入回报增长模板","可持续高增长型"],["小盘高风险模板","小盘高风险型"],["增长模板","增长型"]])text=text.replaceAll(legacy,readable);
...
function verdictModel(r){
  const types=r.types||{};
  const order=Object.keys(TYPE_NAMES);
  const rows=order.map(key=>[key,types[key]||{}]).filter(([,v])=>v&&v.status);
  const actualTriggered=new Set(actualBuyTypes(r));
  const triggered=rows.filter(([key])=>actualTriggered.has(key));
  ...
  const blocked=rows.filter(([,v])=>v.status==="blocked");
  const names=keys=>keys.map(([key])=>(TYPE_NAMES[key]||key).replace(/^[^\s]+\s+/,"").trim()).join("、");
  let tagText,tone,note;
  if(blocked.length){
    tagText="市场状态阻断";tone="blocked";
    note="交易状态阻断："+names(blocked)+"。当前无法形成可执行买入结论，不是买入信号。";
  }else if(triggered.length){
    tagText="模型实际触发候选";tone="buy";
    ...
    tagText="当前不买";tone="neutral";
    note="处于观察区："+names(observed)+"，未达到买入条件。";
  }else{
    tagText="当前不买";tone="neutral";
    note="当前七类均未真实触发，没有买入信号；这不代表卖出。";
  }
  return{tagText,tone,note};
}
function renderVerdict(r){
  ...
  const types=r.types||{};
  const{tagText,tone,note}=verdictModel(r);
...
function renderDetail(r){
  ...
  $("detailMeta").textContent=market(r.code)+" · "+(r.industry||"行业未知")+" · 七类诊断最高分（非买入信号） "+(score(r)>=0?score(r).toFixed(1):"—")+" · 数据日期 "+(marketAsOf||"—");
  ...
  for(const [k,n] of Object.entries(TYPE_NAMES)){
    ...
    const reasonParts=[type7OldData?"服务器仍是旧版第七类数据，等待新数据刷新后再显示结论":publicReasonText(v.reason)||"暂无公司层面补充说明",decisionBasis?"判定依据："+decisionBasis:"",v.patch7_gate?"补丁7红线审计："+publicReasonText(v.patch7_gate):"",v.evidence_gap?"资料缺口："+publicReasonText(v.evidence_gap):""].filter(Boolean);
    ...
    const methodTitle=document.createElement("strong");methodTitle.textContent="这类模型在看什么";
      const thresholds=Array.isArray(method.key_thresholds)?method.key_thresholds.map(String).join("；"):"";const methodText=document.createElement("span");methodText.textContent=(method.summary||"")+" 触发阈值："+(method.trigger||"—")+(thresholds?" 关键门槛："+thresholds+"。":" ")+"计算规则："+(k==="type7"?"7a、7b、7c分别对应本类别的商业模式、护城河、长期成长；质量认证取三项算术平均并严格大于7.000，当前买点还要检查该类别的买点条件和价格。":"类型总分＝各子项分数×对应权重后相加。");
    ...
    for(const [dimension,label] of (TYPE_DIMENSIONS[k]||[])){
      ...
      const missingScore=missing.has(dimension)||!hasScore,positionInstruction=investorActions.has(dimension)&&missing.has(dimension),positionAction=dimension==="6e"&&positionConfirmationRequired(k,v),inactivePositionAction=positionInstruction&&!positionAction,dataMissingScore=missingScore&&!positionInstruction;
      value.textContent=v.status==="not_applicable"?"不适用":positionAction?(v.status==="conditional"?"待仓位确认":"仓位待输入"):inactivePositionAction?"当前无需确认":dataMissingScore?(dimensionEstimatesAvailable&&hasEstimate?"未核验参考 "+Number(estimatedScores[dimension]).toFixed(1)+"分":dimensionScoresAvailable?"资料不足":"数据版本过旧"):Number(subScores[dimension]).toFixed(type7MeanMember?3:1)+"分";
      const evidence=document.createElement("span");evidence.className="dimension-evidence";
      const guidance=v.position_guidance||{};
      const positionEvidence=[guidance.recommendation,guidance.hard_caps,guidance.worst_case_loss].map(publicReasonText).filter(Boolean).join("；")||"请先确认单票仓位、同类组合仓位与最坏归零损失";
      const currentEvidence=positionAction?positionEvidence+(v.status==="conditional"?"":"；仓位输入仍可能改变评分上界，当前尚非买入信号"):inactivePositionAction?"当前未达到进入仓位确认的前置分数条件":publicReasonText(v.reasons?.[dimension]||subReasons[dimension])||cleanedEstimateReason(estimatedReasons[dimension]);
...
async function load(){
  ...
  try{
    ...
    if(String(c.methodology_version||"")!==dataMethodologyVersion)throw new Error("公司索引与数据方法版本不一致，请刷新页面");
    ...
    fillOptions();
    const s=m.summary||{},summary=c.summary||{},scope=analysisScope(m,s.company_count||data.length);renderCoverage(s,summary.type_coverage||{});
    const evidenceGapCompanyCount=Number(summary.evidence_gap_company_count??s.insufficient_company_count??summary.insufficient_company_count??0),decisionRelevantGapCount=Number(summary.decision_relevant_gap_company_count??summary.insufficient_company_count??0),sourceDecisionRelevantCount=Number(summary.source_decision_relevant_company_count??0),proxyDecisionRelevantCount=Number(summary.proxy_decision_relevant_company_count??0),boundedGapCompanyCount=Number(summary.bounded_gap_company_count??Math.max(0,evidenceGapCompanyCount-decisionRelevantGapCount)),actionConfirmationCount=Number(summary.action_confirmation_company_count??s.action_confirmation_company_count??0),{triggeredCompanyCount,conditionalOnlyCompanyCount,pendingOnlyCompanyCount,currentNoBuyCount}=decisionCategoryCounts(data,s);
    const cards=[["模型实际触发候选",triggeredCompanyCount],["待行动确认/附加条件（非信号）",conditionalOnlyCompanyCount],["资料待补候选（非信号）",pendingOnlyCompanyCount],["当前不买（其余公司）",currentNoBuyCount],["结论相关资料待补（并集）",decisionRelevantGapCount],["其中缺直接/结构化证据（可重叠）",sourceDecisionRelevantCount],["其中量化代理待核验（可重叠）",proxyDecisionRelevantCount],["未完全核验但结论已锁定",boundedGapCompanyCount],["其中待确认仓位",actionConfirmationCount]];
    ...
    $("meta").textContent="数据日期："+(m.market_as_of||"—")+" · 更新时间："+formatBeijing(m.data_timestamp_utc)+" · 来源版本："+(sourceVersion||"—")+" · 量化口径："+METHODOLOGY_LABEL+"（"+dataMethodologyVersion+"） · "+analysisScopeText(scope,s.company_count||data.length);
    ...
    if(dataMethodologyVersion!==METHODOLOGY_VERSION)notices.push("当前数据代际并非本页最新规则口径，默认候选仅供旧版研究参考；请等待市场数据重建。");
...
load();
  ...
  .replace("仅情况一满足：排除价值陷阱、衰落产业和长期经营走下坡路。","仅情况一满足：排除价值陷阱、衰落产业和长期经营走下坡路；行业专属增长基准须有中位数和样本量，并有最近连续4年收入，任一证据缺失时资料待补、不默认通过。")
  .replace("所有仍满足条件的类型：必须通过可核验的未来自由现金流前提；明确失败则否决，资料不足则待补。","<strong>七类共同前提：</strong>所有仍满足条件的类型必须通过可核验的未来自由现金流前提；明确失败则否决，资料不足则待补，不能发布为实际触发。")
...
const MARKET_CLOSE_BEIJING_MINUTE=15*60;
...
function dashboardHtmlResponse(request){
  const nonce=cspNonce(),html=INDEX_HTML.replaceAll("__QUANT_CSP_NONCE__",nonce).replace("</style>",".ai-nav{margin:0 0 12px}.ai-nav a{display:inline-block;padding:7px 12px;border:1px solid #bfdbfe;border-radius:999px;background:#eff6ff;color:#1e3a8a;text-decoration:none;font-weight:700}.ai-nav a:hover{background:#dbeafe}</style>").replace("<header id=\"pageHeader\">","<header id=\"pageHeader\"><nav class=\"ai-nav\"><a href=\"/ai-screening\">AI筛查</a></nav>").replace("<script>","<script nonce=\""+nonce+"\">");
...
function legacyAiScreeningPageResponse(request){
  const nonce=cspNonce();
...
function aiScreeningPageResponse(request){
  const nonce=cspNonce();
  const html=`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI筛查 · 量化买入看板</title><style nonce="${nonce}">:root{color-scheme:light;--ink:#172033;--muted:#64748b;--line:#dbe3ef;--bg:#f5f7fb;--panel:#fff;--blue:#1d4ed8}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:15px/1.55 system-ui,-apple-system,"Microsoft YaHei",sans-serif}.wrap{max-width:1180px;margin:auto;padding:20px 20px 48px}.hero,.card{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:20px;box-shadow:0 8px 24px #19324b0b}.hero{display:flex;justify-content:space-between;gap:24px;align-items:end}.hero h1{margin:0 0 6px;font-size:clamp(28px,4vw,44px)}.hero p{margin:0;color:var(--muted);max-width:720px}.back{display:inline-block;margin-bottom:12px;color:var(--blue);font-weight:700;text-decoration:none}.meta{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:8px;color:var(--muted);font-size:12px}.meta span{padding:6px 9px;border-radius:999px;background:#eef4f7}.toolbar{display:flex;gap:12px;flex-wrap:wrap;margin:18px 0}.toolbar label{display:flex;align-items:center;gap:8px;color:var(--muted);font-size:13px}.toolbar select{padding:8px 12px;border:1px solid var(--line);border-radius:10px;background:#fff;color:inherit}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px}.card strong{display:block}.top{display:flex;justify-content:space-between;gap:12px}.top small{color:var(--muted)}.tag{display:inline-block;margin:12px 0 10px;padding:5px 10px;border-radius:999px;font-size:12px;font-weight:700;background:#fff0c7;color:#855b00}.tag.confirmed{background:#def4e5;color:#126a38}.tag.misclassified{background:#ffe0e0;color:#982b2b}.tag.missed_candidate{background:#e9e2ff;color:#5540a4}.tag.needs_review{background:#e2e8f0;color:#475569}.rule,small,.notice{color:var(--muted);font-size:12px}.card p{min-height:48px}.claim{margin:8px 0 0;padding:8px 10px;border-left:3px solid #bfdbfe;background:#f8fbff;font-size:12px}.claim a{color:var(--blue);overflow-wrap:anywhere}.empty{padding:32px;text-align:center;color:var(--muted)}.pager{display:flex;justify-content:space-between;align-items:center;margin-top:14px;color:var(--muted);font-size:13px}.pager button{border:0;border-radius:9px;background:#e2e8f0;color:var(--ink);padding:9px 12px;cursor:pointer;font:inherit}.pager button:disabled{opacity:.45;cursor:default}.disclaimer{margin:18px 0;color:var(--muted);font-size:12px}@media(max-width:700px){.hero{display:block}.meta{justify-content:flex-start;margin-top:18px}.wrap{padding:16px 12px 36px}.pager{gap:8px}}</style></head><body><main class="wrap"><a class="back" href="/">← 返回主看板</a><section class="hero"><div><p>SECOND-PASS RESEARCH</p><h1>AI筛查</h1><p>全部确定性规则已达标或接近达标的候选都会列入；AI只复核财报、公告与反证，不改写七类评分、总闸门或买入状态。</p></div><div class="meta" id="meta"><span>正在读取…</span></div></section><section class="toolbar"><label>结论<select id="verdict"><option value="all">全部候选</option><option value="confirmed">AI复核通过</option><option value="caution">谨慎观察</option><option value="misclassified">疑似误判</option><option value="missed_candidate">疑似漏判</option><option value="needs_review">待本地 AI 复核</option></select></label><label>类型<select id="type"><option value="all">全部类型</option><option>type1</option><option>type2</option><option>type3</option><option>type4</option><option>type5</option><option>type6</option><option>type7</option></select></label></section><section class="grid" id="grid"><p class="empty">正在读取 AI 复核结果…</p></section><nav class="pager" id="pager" hidden><span id="pageInfo"></span><span><button id="prev">上一页</button> <button id="next">下一页</button></span></nav><p class="disclaimer">AI筛查是资料复核和研究排序工具，不是自动买入信号，也不构成收益保证。缺少可核验来源时只能显示待本地 AI 复核；确定性网站结果不受影响。</p></main><script nonce="${nonce}">const esc=v=>String(v??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;");const labels={confirmed:"AI复核通过",caution:"谨慎观察",misclassified:"疑似误判",missed_candidate:"疑似漏判",needs_review:"待本地 AI 复核"};const pageSize=40;let page=1,payload={packets:[]};function filtered(){const verdict=document.querySelector("#verdict").value,type=document.querySelector("#type").value;return (payload.packets||[]).filter(p=>(verdict==="all"||p.ai_review?.verdict===verdict)&&(type==="all"||p.type_key===type))}function render(){const rows=filtered(),pages=Math.max(1,Math.ceil(rows.length/pageSize));page=Math.min(page,pages);const visible=rows.slice((page-1)*pageSize,page*pageSize);document.querySelector("#grid").innerHTML=visible.length?visible.map(p=>{const r=p.ai_review||{},d=p.deterministic||{},v=String(r.verdict||"needs_review"),claims=Array.isArray(r.claims)?r.claims.slice(0,12).map(c=>'<div class="claim">'+esc(c.statement||"")+(c.source_ref?' · <a rel="noreferrer" target="_blank" href="'+esc(c.source_ref)+'">来源</a>':'')+'</div>').join(""):"";return '<article class="card"><div class="top"><strong>'+esc(p.name||p.security_code)+'</strong><small>'+esc(p.security_code)+' · '+esc(p.type_key)+'</small></div><span class="tag '+esc(v)+'">'+esc(labels[v]||v)+'</span><div class="rule">规则状态：'+esc(d.status||"未标注")+' · 分数：'+esc(d.score??d.score_upper_bound??"—")+'</div><p>'+esc(r.summary||"等待本地 AI 复核")+'</p>'+claims+'</article>'}).join(""):'<p class="empty">当前筛选没有候选。</p>';const pager=document.querySelector("#pager");pager.hidden=rows.length===0;document.querySelector("#pageInfo").textContent=rows.length?"显示 "+((page-1)*pageSize+1)+"–"+Math.min(page*pageSize,rows.length)+" / "+rows.length:"";document.querySelector("#prev").disabled=page<=1;document.querySelector("#next").disabled=page>=pages}async function load(){try{const response=await fetch("/api/ai-screening");payload=await response.json();if(!response.ok)throw new Error(payload.error||"暂无结果");const completed=payload.completed_review_count??payload.reviewed_count??0,pending=payload.pending_review_count??0;document.querySelector("#meta").innerHTML='<span>数据代际 '+esc(payload.snapshot_generation||"—")+'</span><span>交易日 '+esc(payload.market_as_of||"—")+'</span><span>AI已完成 '+esc(completed)+' / 候选 '+esc(payload.candidate_total||payload.packets?.length||0)+'</span><span>待复核 '+esc(pending)+'</span>';render()}catch(error){document.querySelector("#meta").innerHTML="<span>AI复核结果暂未发布</span>";document.querySelector("#grid").innerHTML='<p class="empty">'+esc(error.message||"暂无结果")+'。确定性网站结果不受影响。</p>'}}document.querySelector("#verdict").addEventListener("change",()=>{page=1;render()});document.querySelector("#type").addEventListener("change",()=>{page=1;render()});document.querySelector("#prev").addEventListener("click",()=>{page-=1;render()});document.querySelector("#next").addEventListener("click",()=>{page+=1;render()});load();</script></body></html>`;
  const policy="default-src 'none'; base-uri 'none'; connect-src 'self'; form-action 'none'; frame-ancestors 'none'; img-src data:; object-src 'none'; script-src 'nonce-"+nonce+"'; script-src-attr 'none'; style-src 'nonce-"+nonce+"'; style-src-attr 'none'";
  return headSafeResponse(request,new Response(html,{headers:{...BASE_SECURITY_HEADERS,"content-type":"text/html; charset=utf-8","cache-control":"no-store","content-security-policy":policy}}));
}
function aiScreeningPageResponseV2(request){
  const nonce=cspNonce();
  const html=`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI筛查 · 量化买入看板</title><style nonce="${nonce}">:root{color-scheme:light;--ink:#172033;--muted:#64748b;--line:#dbe3ef;--bg:#f5f7fb;--panel:#fff;--blue:#1d4ed8}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:15px/1.55 system-ui,-apple-system,"Microsoft YaHei",sans-serif}.wrap{max-width:1180px;margin:auto;padding:20px 20px 48px}.hero,.card{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:20px;box-shadow:0 8px 24px #19324b0b}.hero{display:flex;justify-content:space-between;gap:24px;align-items:end}.hero h1{margin:0 0 6px;font-size:clamp(28px,4vw,44px)}.hero p{margin:0;color:var(--muted);max-width:760px}.back{display:inline-block;margin-bottom:12px;color:var(--blue);font-weight:700;text-decoration:none}.meta{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:8px;color:var(--muted);font-size:12px}.meta span{padding:6px 9px;border-radius:999px;background:#eef4f7}.toolbar{display:flex;gap:12px;flex-wrap:wrap;margin:18px 0}.toolbar label{display:flex;align-items:center;gap:8px;color:var(--muted);font-size:13px}.toolbar select{padding:8px 12px;border:1px solid var(--line);border-radius:10px;background:#fff;color:inherit}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px}.card strong{display:block}.top{display:flex;justify-content:space-between;gap:12px}.top small{color:var(--muted)}.tag{display:inline-block;margin:12px 0 10px;padding:5px 10px;border-radius:999px;font-size:12px;font-weight:700;background:#fff0c7;color:#855b00}.tag.confirmed{background:#def4e5;color:#126a38}.tag.misclassified{background:#ffe0e0;color:#982b2b}.tag.missed_candidate{background:#e9e2ff;color:#5540a4}.tag.needs_review{background:#fff0c7;color:#855b00}.tag.unreviewed{background:#e2e8f0;color:#475569}.rule,small,.notice{color:var(--muted);font-size:12px}.card p{min-height:48px}.claim{margin:8px 0 0;padding:8px 10px;border-left:3px solid #bfdbfe;background:#f8fbff;font-size:12px}.claim a{color:var(--blue);overflow-wrap:anywhere}.empty{padding:32px;text-align:center;color:var(--muted)}.pager{display:flex;justify-content:space-between;align-items:center;margin-top:14px;color:var(--muted);font-size:13px}.pager button{border:0;border-radius:9px;background:#e2e8f0;color:var(--ink);padding:9px 12px;cursor:pointer;font:inherit}.pager button:disabled{opacity:.45;cursor:default}.disclaimer{margin:18px 0;color:var(--muted);font-size:12px}@media(max-width:700px){.hero{display:block}.meta{justify-content:flex-start;margin-top:18px}.wrap{padding:16px 12px 36px}.pager{gap:8px}}</style></head><body><main class="wrap"><a class="back" href="/">← 返回主看板</a><section class="hero"><div><p>SECOND-PASS RESEARCH</p><h1>AI筛查</h1><p>确定性规则已达标或接近达标的候选都会列入。AI只复核财报、公告与反证，不改写七类评分、总闸门或买入状态。<strong>“证据不足”表示模型已经尝试复核，不是尚未开始。</strong></p></div><div class="meta" id="meta"><span>正在读取…</span></div></section><section class="toolbar"><label>结论<select id="verdict"><option value="all">全部候选</option><option value="confirmed">AI复核通过</option><option value="caution">谨慎观察</option><option value="misclassified">疑似误判</option><option value="missed_candidate">疑似漏判</option><option value="needs_review">AI已尝试·证据不足</option><option value="unreviewed">尚未开始复核</option></select></label><label>类型<select id="type"><option value="all">全部类型</option><option>type1</option><option>type2</option><option>type3</option><option>type4</option><option>type5</option><option>type6</option><option>type7</option></select></label></section><section class="grid" id="grid"><p class="empty">正在读取 AI 复核结果…</p></section><nav class="pager" id="pager" hidden><span id="pageInfo"></span><span><button id="prev">上一页</button> <button id="next">下一页</button></span></nav><p class="disclaimer">AI筛查是资料复核和研究排序工具，不是自动买入信号，也不构成收益保证。模型结论不能自动改变确定性筛选；来源链接仍需人工核对。</p></main><script nonce="${nonce}">const esc=v=>String(v??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;");const placeholderModel="pending-local-opencode-go";const labels={confirmed:"AI复核通过",caution:"谨慎观察",misclassified:"疑似误判",missed_candidate:"疑似漏判",needs_review:"AI已尝试·证据不足",unreviewed:"尚未开始复核"};const pageSize=40;let page=1,payload={packets:[]};function isUnreviewed(packet){return String(packet?.ai_review?.model||"")===placeholderModel}function displayVerdict(packet){return isUnreviewed(packet)?"unreviewed":String(packet?.ai_review?.verdict||"needs_review")}function filtered(){const verdict=document.querySelector("#verdict").value,type=document.querySelector("#type").value;return (payload.packets||[]).filter(packet=>(verdict==="all"||displayVerdict(packet)===verdict)&&(type==="all"||packet.type_key===type))}function render(){const rows=filtered(),pages=Math.max(1,Math.ceil(rows.length/pageSize));page=Math.min(page,pages);const visible=rows.slice((page-1)*pageSize,page*pageSize);document.querySelector("#grid").innerHTML=visible.length?visible.map(packet=>{const review=packet.ai_review||{},deterministic=packet.deterministic||{},verdict=displayVerdict(packet),claims=Array.isArray(review.claims)?review.claims.slice(0,12).map(claim=>'<div class="claim">'+esc(claim.statement||"")+(claim.source_ref?' · <a rel="noreferrer" target="_blank" href="'+esc(claim.source_ref)+'">来源</a>':'')+'</div>').join(""):"";return '<article class="card"><div class="top"><strong>'+esc(packet.name||packet.security_code)+'</strong><small>'+esc(packet.security_code)+' · '+esc(packet.type_key)+'</small></div><span class="tag '+esc(verdict)+'">'+esc(labels[verdict]||verdict)+'</span><div class="rule">规则状态：'+esc(deterministic.status||"未标注")+' · 分数：'+esc(deterministic.score??deterministic.score_upper_bound??"—")+'</div><p>'+esc(review.summary||(isUnreviewed(packet)?"尚未开始本地 AI 复核":"AI已尝试复核，但当前证据不足以给出确认结论。"))+'</p>'+(!isUnreviewed(packet)&&review.model?'<div class="rule">模型：'+esc(review.model)+' · 强度：'+esc(review.effort||"—")+'</div>':'')+claims+'</article>'}).join(""):"<p class=\"empty\">当前筛选没有候选。</p>";const pager=document.querySelector("#pager");pager.hidden=rows.length===0;document.querySelector("#pageInfo").textContent=rows.length?"显示 "+((page-1)*pageSize+1)+"–"+Math.min(page*pageSize,rows.length)+" / "+rows.length:"";document.querySelector("#prev").disabled=page<=1;document.querySelector("#next").disabled=page>=pages}async function load(){try{const response=await fetch("/api/ai-screening");const value=await response.json();if(!response.ok)throw new Error(value.error||"暂无结果");payload=value;const attempted=Number(value.attempted_review_count??value.completed_review_count??0),unreviewed=Number(value.unreviewed_candidate_count??value.pending_review_count??0),insufficient=Number(value.attempted_needs_review_count??0);document.querySelector("#meta").innerHTML='<span>数据代际 '+esc(value.snapshot_generation||"—")+'</span><span>交易日 '+esc(value.market_as_of||"—")+'</span><span>AI已尝试 '+esc(attempted)+' / 候选 '+esc(value.candidate_total||value.packets?.length||0)+'</span><span>证据不足 '+esc(insufficient)+'</span><span>尚未开始 '+esc(unreviewed)+'</span>';render()}catch(error){document.querySelector("#meta").innerHTML="<span>AI复核结果暂未发布</span>";document.querySelector("#grid").innerHTML='<p class="empty">'+esc(error.message||"暂无结果")+'。确定性网站结果不受影响。</p>'}}document.querySelector("#verdict").addEventListener("change",()=>{page=1;render()});document.querySelector("#type").addEventListener("change",()=>{page=1;render()});document.querySelector("#prev").addEventListener("click",()=>{page-=1;render()});document.querySelector("#next").addEventListener("click",()=>{page+=1;render()});load();</script></body></html>`;
  const policy="default-src 'none'; base-uri 'none'; connect-src 'self'; form-action 'none'; frame-ancestors 'none'; img-src data:; object-src 'none'; script-src 'nonce-"+nonce+"'; script-src-attr 'none'; style-src 'nonce-"+nonce+"'; style-src-attr 'none'";
  return headSafeResponse(request,new Response(html.replace('<p class="empty">当前筛选没有候选。</p>','<p class=empty>当前筛选没有候选。</p>'),{headers:{...BASE_SECURITY_HEADERS,"content-type":"text/html; charset=utf-8","cache-control":"no-store","content-security-policy":policy}}));
}
function aiScreeningPageResponseV3(request){
  const nonce=cspNonce();
  const html=`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI优中选优 · 量化买入看板</title><style nonce="${nonce}">:root{color-scheme:light;--ink:#172033;--muted:#64748b;--line:#dbe3ef;--bg:#f5f7fb;--panel:#fff;--blue:#1d4ed8;--green:#147a46;--amber:#996515;--red:#a12626}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:15px/1.55 system-ui,-apple-system,"Microsoft YaHei",sans-serif}.wrap{max-width:1240px;margin:auto;padding:20px 20px 54px}.hero,.card,.stat{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:20px;box-shadow:0 8px 24px #19324b0b}.hero{display:flex;justify-content:space-between;gap:24px;align-items:end}.hero h1{margin:0 0 6px;font-size:clamp(28px,4vw,44px)}.hero p{margin:0;color:var(--muted);max-width:800px}.back{display:inline-block;margin-bottom:12px;color:var(--blue);font-weight:700;text-decoration:none}.meta,.stats{display:flex;flex-wrap:wrap;gap:8px}.meta{justify-content:flex-end;color:var(--muted);font-size:12px}.meta span,.stat{padding:8px 11px;border-radius:10px;background:#eef4f7}.stats{margin:18px 0}.stat{min-width:150px}.stat b{display:block;font-size:24px}.stat small{color:var(--muted)}.toolbar{display:flex;gap:12px;flex-wrap:wrap;margin:18px 0}.toolbar label{display:flex;align-items:center;gap:8px;color:var(--muted);font-size:13px}.toolbar select,.toolbar input{padding:8px 12px;border:1px solid var(--line);border-radius:10px;background:#fff;color:inherit}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(340px,1fr));gap:14px}.card{padding:18px}.top{display:flex;justify-content:space-between;gap:12px}.top strong{font-size:18px}.top small,.rule,.confidence{color:var(--muted);font-size:12px}.scoreline{display:flex;align-items:center;gap:10px;margin:12px 0}.score{font-size:32px;font-weight:800;color:var(--blue)}.action,.confidence{display:inline-block;padding:4px 9px;border-radius:999px;background:#e2e8f0}.action.priority_buy{background:#def4e5;color:var(--green);font-weight:700}.action.watchlist{background:#fff0c7;color:var(--amber);font-weight:700}.action.avoid{background:#ffe0e0;color:var(--red);font-weight:700}.action.insufficient_evidence{background:#e2e8f0;color:#475569}.section{margin-top:12px}.section h3{margin:0 0 5px;font-size:13px}.section ul{margin:5px 0 0;padding-left:20px}.claim{margin:8px 0 0;padding:8px 10px;border-left:3px solid #bfdbfe;background:#f8fbff;font-size:12px}.claim a{color:var(--blue);overflow-wrap:anywhere}.empty{padding:32px;text-align:center;color:var(--muted)}.pager{display:flex;justify-content:space-between;align-items:center;margin-top:14px;color:var(--muted);font-size:13px}.pager button{border:0;border-radius:9px;background:#e2e8f0;color:var(--ink);padding:9px 12px;cursor:pointer;font:inherit}.pager button:disabled{opacity:.45;cursor:default}.notice{margin:18px 0;padding:12px 14px;background:#fff;border:1px solid var(--line);border-radius:12px;color:var(--muted);font-size:12px}@media(max-width:700px){.hero{display:block}.meta{justify-content:flex-start;margin-top:18px}.wrap{padding:16px 12px 36px}.grid{grid-template-columns:1fr}.pager{gap:8px}}</style></head><body><main class="wrap"><a class="back" href="/">← 返回主看板</a><section class="hero"><div><p>AI SECOND-PASS RANKING</p><h1>AI优中选优</h1><p>所有确定性规则已达标或接近达标的候选都会先进入池子，再由 AI 结合估值、现金流、成长/周期、竞争力、催化剂和反证做第二轮排序。页面按“AI买入吸引力分”从高到低展示；它不改写确定性规则，也不自动下单。</p></div><div class="meta" id="meta"><span>正在读取…</span></div></section><section class="stats" id="stats"></section><div class="notice">“AI优先候选”表示已通过联网资料核验、且确定性规则已触发的优先研究对象，不代表无风险或自动买入。未联网或来源不足的候选只保留为待核验，不进入优先候选。</div><section class="toolbar"><label>AI结论<select id="action"><option value="all">全部</option><option value="priority_buy">AI优先候选</option><option value="watchlist">观察清单</option><option value="insufficient_evidence">待核验/未形成买入结论</option><option value="avoid">不建议</option></select></label><label>类型<select id="type"><option value="all">全部类型</option><option>type1</option><option>type2</option><option>type3</option><option>type4</option><option>type5</option><option>type6</option><option>type7</option></select></label><label>最低分<input id="minScore" type="number" min="0" max="100" step="1" value="0"></label></section><section class="grid" id="grid"><p class="empty">正在读取 AI 优中选优结果…</p></section><nav class="pager" id="pager" hidden><span id="pageInfo"></span><span><button id="prev">上一页</button> <button id="next">下一页</button></span></nav></main><script nonce="${nonce}">const esc=v=>String(v??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;");const labels={priority_buy:"AI优先候选",watchlist:"观察清单",insufficient_evidence:"待核验",avoid:"不建议"};const pageSize=30;let page=1,payload={packets:[]};function filtered(){const action=document.querySelector("#action").value,type=document.querySelector("#type").value,min=Math.max(0,Number(document.querySelector("#minScore").value||0));return(payload.packets||[]).filter(packet=>{const review=packet.ai_review||{};return(action==="all"||review.ai_action===action)&&(type==="all"||packet.type_key===type)&&Number(review.buy_attractiveness_score||0)>=min})}function list(values,title){return Array.isArray(values)&&values.length?'<div class="section"><h3>'+title+'</h3><ul>'+values.slice(0,8).map(value=>'<li>'+esc(value)+'</li>').join("")+'</ul></div>':''}function render(){const rows=filtered(),pages=Math.max(1,Math.ceil(rows.length/pageSize));page=Math.min(page,pages);const visible=rows.slice((page-1)*pageSize,page*pageSize);document.querySelector("#grid").innerHTML=visible.length?visible.map(packet=>{const review=packet.ai_review||{},det=packet.deterministic||{},action=String(review.ai_action||"insufficient_evidence"),claims=Array.isArray(review.claims)?review.claims.slice(0,8).map(claim=>'<div class="claim">'+esc(claim.statement||"")+(claim.source_ref?' · <a rel="noreferrer" target="_blank" href="'+esc(claim.source_ref)+'">来源</a>':'')+'</div>').join(""):'';return'<article class="card"><div class="top"><strong>#'+esc(packet.ai_rank||"—")+' '+esc(packet.name||packet.security_code)+'</strong><small>'+esc(packet.security_code)+' · '+esc(packet.type_key)+'</small></div><div class="scoreline"><span class="score">'+esc(Number(review.buy_attractiveness_score||0).toFixed(1))+'</span><span class="action '+esc(action)+'">'+esc(labels[action]||action)+'</span><span class="confidence">确定性：'+esc(review.confidence||"—")+'</span><span class="confidence">来源：'+(review.web_search_verified?"已联网核验":(review.web_search_performed?"已搜索但无可用链接":"未联网核验"))+'</span></div><div class="rule">确定性规则：'+esc(det.status||"—")+' · 规则分数：'+esc(det.score??det.score_upper_bound??"—")+' · AI原结论：'+esc(review.verdict||"—")+'</div><p>'+esc(review.summary||"没有 AI 说明")+'</p>'+list(review.key_strengths,"为什么进入优先级")+list(review.risk_flags,"主要风险/待核验")+claims+'</article>'}).join(""):'<p class="empty">当前筛选没有候选。</p>';const pager=document.querySelector("#pager");pager.hidden=rows.length===0;document.querySelector("#pageInfo").textContent=rows.length?"显示 "+((page-1)*pageSize+1)+"–"+Math.min(page*pageSize,rows.length)+" / "+rows.length:"";document.querySelector("#prev").disabled=page<=1;document.querySelector("#next").disabled=page>=pages}async function load(){try{const response=await fetch("/api/ai-screening");const value=await response.json();if(!response.ok)throw new Error(value.error||"暂无结果");payload=value;const counts=value.ai_action_counts||{};document.querySelector("#meta").innerHTML='<span>数据代际 '+esc(value.snapshot_generation||"—")+'</span><span>交易日 '+esc(value.market_as_of||"—")+'</span><span>候选 '+esc(value.candidate_total||value.packets?.length||0)+'</span><span>已联网核验 '+esc(value.web_search_completed_count||0)+'</span>';document.querySelector("#stats").innerHTML='<div class="stat"><b>'+esc(value.priority_buy_count||0)+'</b><small>AI优先候选</small></div><div class="stat"><b>'+esc(value.watchlist_count||counts.watchlist||0)+'</b><small>观察清单</small></div><div class="stat"><b>'+esc(value.insufficient_evidence_count||counts.insufficient_evidence||0)+'</b><small>待核验（未形成买入结论）</small></div><div class="stat"><b>'+esc(value.web_search_completed_count||0)+'</b><small>已联网核验</small></div><div class="stat"><b>'+esc(value.avoid_count||counts.avoid||0)+'</b><small>不建议</small></div>';render()}catch(error){document.querySelector("#meta").innerHTML="<span>AI优中选优结果暂未发布</span>";document.querySelector("#grid").innerHTML='<p class="empty">'+esc(error.message||"暂无结果")+'。确定性网站结果不受影响。</p>'}}document.querySelector("#action").addEventListener("change",()=>{page=1;render()});document.querySelector("#type").addEventListener("change",()=>{page=1;render()});document.querySelector("#minScore").addEventListener("input",()=>{page=1;render()});document.querySelector("#prev").addEventListener("click",()=>{page-=1;render()});document.querySelector("#next").addEventListener("click",()=>{page+=1;render()});load();</script></body></html>`;
  const policy="default-src 'none'; base-uri 'none'; connect-src 'self'; form-action 'none'; frame-ancestors 'none'; img-src data:; object-src 'none'; script-src 'nonce-"+nonce+"'; script-src-attr 'none'; style-src 'nonce-"+nonce+"'; style-src-attr 'none'";
  return headSafeResponse(request,new Response(html,{headers:{...BASE_SECURITY_HEADERS,"content-type":"text/html; charset=utf-8","cache-control":"no-store","content-security-policy":policy}}));
}
function validAiScreeningArtifact(value,generation){
  ...
  const hasAttemptedFields=value.attempted_review_count!==undefined||value.unreviewed_candidate_count!==undefined;
  const attempted=Number(value.attempted_review_count??value.completed_review_count??value.reviewed_count),unreviewed=Number(value.unreviewed_candidate_count??value.pending_review_count??0);
  if(!Number.isInteger(attempted)||attempted<0||!Number.isInteger(unreviewed)||unreviewed<0||attempted+unreviewed!==value.packets.length)return false;
  if(hasAttemptedFields&&(!Number.isInteger(Number(value.attempted_needs_review_count??0))||Number(value.attempted_needs_review_count)<0))return false;
  const seen=new Set();
...
function sameDayPostClosePublication(marketAsOf,timestamp,nowMs=Date.now()){
  const nowClock=beijingClock(nowMs),generatedMs=Date.parse(String(timestamp||"")),generatedClock=beijingClock(generatedMs);
  return Boolean(nowClock&&generatedClock&&generatedMs<=nowMs&&nowClock.minute>=MARKET_CLOSE_BEIJING_MINUTE&&marketAsOf===nowClock.date&&generatedClock.date===marketAsOf&&generatedClock.minute>=MARKET_CLOSE_BEIJING_MINUTE&&isExpectedTradingDay(marketAsOf));
}
...
function tradingDataFreshness(marketAsOf,timestamp,nowMs=Date.now()){
  const scheduledExpectedMarketAsOf=latestExpectedClosedTradingDate(nowMs),postCloseToday=sameDayPostClosePublication(marketAsOf,timestamp,nowMs),expectedMarketAsOf=postCloseToday?marketAsOf:scheduledExpectedMarketAsOf,dataAgeHours=boundedDataAgeHours(timestamp,nowMs),marketDateValid=validIsoDate(marketAsOf),marketDateCurrent=Boolean(marketDateValid&&expectedMarketAsOf&&marketAsOf===expectedMarketAsOf),withinHardAge=Boolean(dataAgeHours!==null&&dataAgeHours<=MAX_TRADING_DATA_AGE_HOURS);
...
function proxyVerificationDimensions(value,dimensions){const reasons=value?.estimated_sub_score_reasons;return dimensions.filter(dimension=>String(reasons?.[dimension]||"").includes("模型代理证据，最高4分"))}
function typeDataGap(typeKey,value){const decision=value?.decision||{},missing=Array.isArray(decision.missing_dimensions)?decision.missing_dimensions:[],actionDimensions=investorActionDimensions(typeKey,value),dataMissing=missing.filter(dimension=>!actionDimensions.has(dimension)),actionOnly=missing.length>0&&dataMissing.length===0,actionRequired=actionDimensions.has("6e")&&missing.includes("6e")&&value?.status==="conditional",declaredIncomplete=value?.applicable===true&&value?.evidence_complete===false,hasGap=dataMissing.length>0||(!actionOnly&&(declaredIncomplete||Boolean(String(value?.evidence_gap||"").trim()))),proxyDimensions=proxyVerificationDimensions(value,dataMissing),proxySet=new Set(proxyDimensions),sourceDimensions=dataMissing.filter(dimension=>!proxySet.has(dimension)),proxyVerification=hasGap&&proxyDimensions.length>0,sourceGap=hasGap&&(sourceDimensions.length>0||dataMissing.length===0),decisionRelevant=hasGap&&decision.decision_complete===false;return{has_gap:hasGap,source_gap:sourceGap,proxy_verification:proxyVerification,decision_relevant:decisionRelevant,bounded:hasGap&&!decisionRelevant,action_required:actionRequired}}
function compactType(typeKey,value){const decision=value?.decision||{},gap=typeDataGap(typeKey,value);return{status:String(value?.status||"invalid"),score:value?.score??null,...(typeKey==="type6"&&value?.diagnostic_score!==undefined?{diagnostic_score:value.diagnostic_score}:{}),...(typeKey==="type7"&&value?.quality_complete!==undefined?{quality_complete:value.quality_complete===true,quality_certified:value.quality_certified===true,buy_ready:value.buy_ready===true}:{}),score_lower_bound:decision.score_lower_bound??null,score_upper_bound:decision.score_upper_bound??null,has_missing_dimensions:Array.isArray(decision.missing_dimensions)&&decision.missing_dimensions.length>0,has_evidence_gap:gap.has_gap,source_gap:gap.source_gap,proxy_verification:gap.proxy_verification,decision_relevant:gap.decision_relevant,bounded:gap.bounded}}
function compactCompany(company){const types={};for(const key of Object.keys(company?.types||{}))types[key]=compactType(key,company.types[key]);return{code:String(company?.code||""),name:String(company?.name||""),industry:String(company?.industry||""),diagnostic_score:company?.diagnostic_score??null,buy_types:Array.isArray(company?.buy_types)?company.buy_types.map(String):[],primary_type:String(company?.primary_type||""),primary_label:String(company?.primary_label||""),types}}
...
export default{
  async fetch(request,env){
    ...
    try{
      ...
      if(path==="/ai-screening")return aiScreeningPageResponseV3(request);
      ...
      if(path==="/api/catalogue-index"){
        const manifest=await generationManifest(env,generation);const build=async()=>{const catalogue=await readCatalogue(env,prefix,manifest),methodologyVersion=generationMethodologyVersion(manifest);return{index_contract:CATALOGUE_INDEX_CONTRACT_VERSION,generation_id:generation.generation_id,methodology_version:methodologyVersion,methodology_current:methodologyVersion===METHODOLOGY_VERSION,capabilities:catalogue.capabilities||{},summary:{company_count:catalogue.companies.length,...catalogueGapSummary(catalogue.companies)},companies:catalogue.companies.map(compactCompany)}};return immutableProjection(request,build,catalogueIndexCacheRequest(request,generation.generation_id));
      }
