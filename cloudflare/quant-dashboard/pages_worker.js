...
function validAiScreeningArtifact(value,generation){
  ...
  const completed=Number(value.completed_review_count??value.reviewed_count),pending=Number(value.pending_review_count??0);
  if(!Number.isInteger(completed)||completed<0||!Number.isInteger(pending)||pending<0||completed+pending!==value.packets.length)return false;
  const seen=new Set();
