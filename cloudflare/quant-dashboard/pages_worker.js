...
function rowMatches(r,{q,m,t,s,i}){const typeState=t?(r.types?.[t]||null):null;const typeMatches=!t||(typeState&&((s&&typeStatusMatches(typeState,s))||(!s&&typeState.status!=="not_applicable")));const statusMatches=!s||t||Object.values(r.types||{}).some(value=>typeStatusMatches(value,s));return(!q||r._search.includes(q))&&(!m||r._market===m)&&(!i||r.industry===i)&&typeMatches&&statusMatches}
...
function renderCoverage(summary,indexCoverage={}){const total=Number(summary.company_count||data.length)||1;const coverage=summary.type_coverage||{};const nodes=Object.entries(TYPE_NAMES).map(([key,name])=>{const record=coverage[key]||{},evidence=coverageEvidenceStats(record,indexCoverage?.[key]);const triggered=Number(record.triggered||0),conditional=Number(record.conditional||0),visibleCount=triggered+conditional,conditionalLabel=conditionalCoverageLabel(key,conditional,evidence.actionConfirmation),evidenceLabel="资料缺口 "+evidence.evidenceMissing.toLocaleString()+" 家（其中结论待定 "+evidence.decisionUnresolved.toLocaleString()+" 家，其中补齐后仍可能触发 "+evidence.potentiallyTriggerable.toLocaleString()+" 家）";const preferredStatus=triggered>0?"triggered":conditional>0?"conditional":evidence.evidenceMissing>0?"evidence_gap":"";const button=document.createElement("button");button.type="button";button.dataset.type=key;button.dataset.status=preferredStatus;button.setAttribute("aria-label",name+"，已触发 "+triggered+" 家，"+conditionalLabel+"，"+evidenceLabel);const label=document.createElement("span");label.className="coverage-label coverage-triggered";label.append(document.createTextNode(name.replace(/^[^ ]+ /,"")),document.createTextNode(triggered.toLocaleString()+" 家已触发"));const track=document.createElement("span");track.className="coverage-track";const fill=document.createElement("span");fill.className="coverage-fill";fill.style.width=Math.max(0,Math.min(100,visibleCount/total*100))+"%";if(visibleCount===0)fill.hidden=true;track.append(fill);const breakdown=document.createElement("span");breakdown.className="coverage-breakdown";const conditionalText=document.createElement("span");conditionalText.className="coverage-conditional";conditionalText.textContent=conditionalLabel;const evidenceText=document.createElement("span");evidenceText.className="coverage-evidence";evidenceText.textContent=evidenceLabel;breakdown.append(conditionalText,evidenceText);button.append(label,track,breakdown);return button});$("coverage").replaceChildren(...nodes)}
...
function renderDetail(r){
  ...
  for(const [k,n] of Object.entries(TYPE_NAMES)){
    ...
    for(const [dimension,label] of (TYPE_DIMENSIONS[k]||[])){
      ...
      value.textContent=v.status==="not_applicable"?"不适用":positionAction?"待仓位确认":inactivePositionAction?"当前无需确认":dataMissingScore?(dimensionEstimatesAvailable&&hasEstimate?"未核验参考 "+Number(estimatedScores[dimension]).toFixed(1)+"分":dimensionScoresAvailable?"资料不足":"数据版本过旧"):Number(subScores[dimension]).toFixed(1)+"分";
      const evidence=document.createElement("span");evidence.className="dimension-evidence";
      const guidance=v.position_guidance||{};
      const currentEvidence=positionAction?[guidance.recommendation,guidance.hard_caps,guidance.worst_case_loss].filter(Boolean).join("；")||"请先确认单票仓位、同类组合仓位与最坏归零损失":inactivePositionAction?"当前未达到进入仓位确认的前置分数条件":v.reasons?.[dimension]||subReasons[dimension]||cleanedEstimateReason(estimatedReasons[dimension]);
      ...
      if(hasScore&&!dataMissingScore&&!positionInstruction&&v.status!=="not_applicable"&&!independentGate)addDefinition(definitions,"总分贡献",(Number(subScores[dimension])*weight/100).toFixed(2)+"分（"+Number(subScores[dimension]).toFixed(1)+" × "+weight.toFixed(weight%1?1:0)+"%）","contribution");
      else if(hasScore&&!dataMissingScore&&!positionInstruction&&v.status!=="not_applicable"&&independentGate)addDefinition(definitions,"门槛结果",Number(subScores[dimension]).toFixed(3)+"分，"+(Number(subScores[dimension])>7?"已严格高于7.0分":"未严格高于7.0分")+"；该项单独判定。","gate-result");
