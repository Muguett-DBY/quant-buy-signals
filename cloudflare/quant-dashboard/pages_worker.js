...
function validAiScreeningArtifact(value,generation){
  ...
  for(const packet of value.packets){const code=String(packet?.security_code||""),type=String(packet?.type_key||""),review=packet?.ai_review,model=String(review?.model||"");if(!/^[036]\d{5}$/.test(code)||!/^type[1-7]$/.test(type)||seen.has(code+"/"+type)||!review||typeof review!=="object"||!["confirmed","caution","misclassified","missed_candidate","needs_review"].includes(String(review.verdict))||!["keep","demote","manual_review"].includes(String(review.recommended_action))||!Array.isArray(review.claims)||review.claims.length>12)return false;if(model==="pending-local-opencode-go"){if(String(review.verdict)!=="needs_review")return false;actualUnreviewed++}else{actualAttempted++;if(String(review.verdict)==="needs_review")actualNeedsReview++}seen.add(code+"/"+type)}
...
export default{
  async fetch(request,env){
    ...
    try{
      ...
      if(path==="/ai-screening")return aiScreeningPageResponseV2(request);
