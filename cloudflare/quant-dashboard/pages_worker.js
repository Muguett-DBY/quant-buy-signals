...
function detail(r){
  ...
  for(const [k,n] of Object.entries(TYPE_NAMES)){
    ...
    for(const [dimension,label] of (TYPE_DIMENSIONS[k]||[])){
      ...
      const hasEstimate=Object.prototype.hasOwnProperty.call(estimatedScores,dimension)&&Number.isFinite(Number(estimatedScores[dimension]));
      const missingScore=missing.has(dimension)||!hasScore;
      const positionAction=dimension==="6e"&&v.action_required==="position_confirmation";
      value.textContent=v.status==="not_applicable"?"不适用":positionAction?"待仓位确认":missingScore?(dimensionEstimatesAvailable&&hasEstimate?"未确认估算 "+Number(estimatedScores[dimension]).toFixed(1)+"分":dimensionScoresAvailable?"资料不足":"数据版本过旧"):Number(subScores[dimension]).toFixed(1)+"分";
      const evidence=document.createElement("em");
      const guidance=v.position_guidance||{};
      evidence.textContent=positionAction?[guidance.recommendation,guidance.hard_caps,guidance.worst_case_loss].filter(Boolean).join("；"):subReasons[dimension]||estimatedReasons[dimension]||(v.status==="not_applicable"?"该类型不适用于此公司":missingScore?"尚未取得可核验的该项资料":dimensionScoresAvailable?"服务器未提供有效说明":"请等待最新数据发布");
