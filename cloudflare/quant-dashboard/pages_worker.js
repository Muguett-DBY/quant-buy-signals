const METHODOLOGY_VERSION="patch6-seven-types-2026-07-30";
const METHODOLOGY_LABEL="七类量化买入方法（2026年7月）";
const METHODOLOGY={
  type1:{
  ...
  type3:{
    ...
    applicability:"仅适用于经连续历史确认、趋势增长达到10%的公司；低于该边界属于模型不适用而非数据缺失。",
    trigger:"证据完整、加权总分至少7分，护城河不能过弱；泡沫过高会压低或否决结果。",
    dimensions:{
  ...
  type5:{
    ...
    trigger:"证据完整、加权总分至少7分，并且强周期属性至少7分；抗周期能力参与加权但不是单独硬门槛。",
    dimensions:{
  ...
  type6:{
    ...
    dimensions:{
      ...
      "6b":{weight:20,meaning:"核心技术是否真实、可追溯且难以复制。",data:"研发、专利、产品验证和公告或报告中的直接技术证据。",direction:"只有可追溯原始资料可以给出精确分，故事性描述不计分。"},
      "6c":{weight:15,meaning:"商业模式是否带来可验证的单位经济或扩张优势。",data:"客户、收入模式、单位经济、复购与可追溯商业模式证据。",direction:"可验证的模式创新和改善越强，分数越高。"},
  ...
  type7:{
    summary:"把长期质量、产业与估值、商业质量与安全边际三套独立评分同时作为优质股权门槛。",
    applicability:"适用于非金融公司；金融股需要尚未建立的专属优质股权模型。",
    trigger:"三套百分制账本都必须证据充分且严格高于70分；任一项不超过70分都不能触发。",
    dimensions:{
      "7a":{weight:100/3,meaning:"长期经营质量和股东实际回报。",data:"长期每股价值、资本回报、现金质量、股东回报与五至十年市场历史。",direction:"底层长期质量与回报账本除以10形成0至10分。"},
      "7b":{weight:100/3,meaning:"产业质量、竞争位置与当前估值的组合。",data:"行业质量、竞争格局、增长耐久性、五年估值分位与十年估值证据。",direction:"底层产业质量与估值账本除以10形成0至10分。"},
      "7c":{weight:100/3,meaning:"商业模式质量与买入安全边际。",data:"商业安全、资本配置、竞争风险、公告与研究资料、估值安全边际。",direction:"底层商业安全账本除以10形成0至10分，并执行安全否决。"}
    }
  }
};
...
const $=id=>document.getElementById(id);const pageSize=50;let data=[],page=0,generationId="",marketAsOf="",sourceVersion="",dimensionScoresAvailable=false,dimensionEstimatesAvailable=false,returnFocus=null,composing=false,renderFrame=0,activeDetailRequest=0,activeDetailCode="",detailAbort=null;const detailCache=new Map();
function market(code){return String(code).startsWith("6")?"沪市":"深市"}
function score(row,type=""){const result=type?row.types?.[type]:null;const value=finiteNumber(type?result?.score:row.diagnostic_score);if(value!==null)return value;const upper=finiteNumber(result?.score_upper_bound??result?.decision?.score_upper_bound);return type&&upper!==null?upper:-1}
function scoreLabel(typeResult){if(!typeResult||typeResult.status==="not_applicable")return"";const hasMissing=typeResult.has_missing_dimensions===true||(typeResult.decision?.missing_dimensions||[]).length>0;const exact=finiteNumber(typeResult.score);if(exact!==null&&!hasMissing)return exact.toFixed(1)+"分";const lower=finiteNumber(typeResult.score_lower_bound??typeResult.decision?.score_lower_bound),upper=finiteNumber(typeResult.score_upper_bound??typeResult.decision?.score_upper_bound);if(lower!==null&&upper!==null&&lower<=upper)return Math.abs(upper-lower)<.05?upper.toFixed(1)+"分":"参考范围 "+lower.toFixed(1)+"–"+upper.toFixed(1)+"分";return"资料未齐，暂不显示精确分"}
...
function rowMatches(r,{q,m,t,s,i}){const typeState=t?(r.types?.[t]||null):null;const typeMatches=!t||(typeState&&((s&&typeState.status===s)||(!s&&typeState.status!=="not_applicable")));const statusMatches=!s||t||Object.values(r.types||{}).some(v=>v.status===s);return(!q||r._search.includes(q))&&(!m||r._market===m)&&(!i||r.industry===i)&&typeMatches&&statusMatches}
...
function badge(status,text=""){const span=document.createElement("span");span.className="status s-"+status;span.textContent=text||STATUS_NAMES[status]||"资料异常";return span}
function renderCoverage(summary){const total=Number(summary.company_count||data.length)||1;const coverage=summary.type_coverage||{};const nodes=Object.entries(TYPE_NAMES).map(([key,name])=>{const record=coverage[key]||{};const triggered=Number(record.triggered||0),conditional=Number(record.conditional||0),visibleCount=triggered+conditional,conditionalLabel=key==="type6"?"等待确认仓位":"待满足附加条件";const preferredStatus=triggered>0?"triggered":conditional>0?"conditional":"";const button=document.createElement("button");button.type="button";button.dataset.type=key;button.dataset.status=preferredStatus;button.setAttribute("aria-label",name+"，"+triggered+"家已触发"+(conditional?"，"+conditional+"家"+conditionalLabel:""));const label=document.createElement("span");label.className="coverage-label";label.append(document.createTextNode(name.replace(/^[^ ]+ /,"")),document.createTextNode(triggered.toLocaleString()+" 家已触发"));const track=document.createElement("span");track.className="coverage-track";const fill=document.createElement("span");fill.className="coverage-fill";fill.style.width=Math.max(0,Math.min(100,visibleCount/total*100))+"%";if(visibleCount===0)fill.hidden=true;track.append(fill);const small=document.createElement("small");small.textContent=conditional?conditional.toLocaleString()+" 家"+conditionalLabel:"点击查看公司名单";button.append(label,track,small);return button});$("coverage").replaceChildren(...nodes)}
...
function cleanedEstimateReason(value){return String(value||"").replace(/^未确认估算，不用于触发[；;]?/,"").trim()}
...
function renderDetail(r){
  ...
  const help=document.createElement("p");help.className="detail-help";help.textContent="每个子指标先显示当前分数与公司实际依据；展开后可查看所需数据、公开详情提供的实际输入、数据批次、来源追溯、阈值和计算方式。没有公开的报告期或单项来源会明确写明，不会猜测补全。";
  ...
  fragment.append(facts,help,nav);
  for(const [k,n] of Object.entries(TYPE_NAMES)){
    const v=r.types?.[k]||{},method=METHODOLOGY[k]||{dimensions:{}};
    ...
    const typeScoreLabel=scoreLabel(v);if(typeScoreLabel)right.append(document.createTextNode(k==="type7"?" 综合诊断 "+typeScoreLabel+"（不替代三项门槛）":" "+typeScoreLabel));
    const reason=document.createElement("p");
    const decisionBasis=DECISION_BASIS_NAMES[v.decision?.decision_basis]||"";
    const reasonParts=[v.reason||"暂无公司层面补充说明",decisionBasis?"判定依据："+decisionBasis:"",v.evidence_gap?"资料缺口："+v.evidence_gap:""].filter(Boolean);
    ...
    const methodText=document.createElement("span");methodText.textContent=(method.summary||"")+" 触发阈值："+(method.trigger||"—")+" 计算规则："+(k==="type7"?"7a、7b、7c分别严格高于7.0分，任一项不达标都不能触发，三项不可相互抵消。":"类型总分＝各子项分数×对应权重后相加。");
    ...
    scope.textContent=v.status==="not_applicable"?(coverageGap?"模型覆盖缺口："+(v.reason||method.applicability||"当前没有适用的专属模型")+"。这不是公司得0分，也不是已被否决；需要建立相应专属模型后才能评价。":"为什么不适用："+(v.reason||method.applicability||"超出模型适用范围")+"。这是公司的适用边界，不是缺失公司资料，系统不会用0分伪装成否决。"):"适用范围："+(method.applicability||"—");
    ...
    for(const [key,value] of Object.entries(v.reasons||{})){if(dimensionKeys.has(key)||!EVIDENCE_META_NAMES[key]||!String(value||"").trim())continue;const item=document.createElement("div");item.className="evidence-note";const label=document.createElement("strong");label.textContent=EVIDENCE_META_NAMES[key];const text=document.createElement("span");text.textContent=String(value);item.append(label,text);evidenceNotes.append(item)}
    const dimensions=document.createElement("div");dimensions.className="dimensions";
    const subScores=v.sub_scores||{},subReasons=v.sub_score_reasons||{},estimatedScores=v.estimated_sub_scores||{},estimatedReasons=v.estimated_sub_score_reasons||{},missing=new Set(v.decision?.missing_dimensions||[]);
    for(const [dimension,label] of (TYPE_DIMENSIONS[k]||[])){
      const item=document.createElement("details");item.className="dimension";
      const dimensionMethod=method.dimensions?.[dimension]||{},weight=Number(dimensionMethod.weight||0),independentGate=k==="type7";
      const summary=document.createElement("summary");
      const name=document.createElement("span");name.className="dimension-title";name.textContent=dimension+" · "+label+(independentGate?" · 独立门槛：严格高于7.0分":" · 权重"+weight.toFixed(weight%1?1:0)+"%");
      ...
      const missingScore=missing.has(dimension)||!hasScore,positionAction=dimension==="6e"&&v.action_required==="position_confirmation";
      value.textContent=v.status==="not_applicable"?"不适用":positionAction?"待仓位确认":missingScore?(dimensionEstimatesAvailable&&hasEstimate?"未核验参考 "+Number(estimatedScores[dimension]).toFixed(1)+"分":dimensionScoresAvailable?"资料不足":"数据版本过旧"):Number(subScores[dimension]).toFixed(1)+"分";
      const evidence=document.createElement("span");evidence.className="dimension-evidence";
      const guidance=v.position_guidance||{};
      const currentEvidence=positionAction?[guidance.recommendation,guidance.hard_caps,guidance.worst_case_loss].filter(Boolean).join("；"):v.reasons?.[dimension]||subReasons[dimension]||cleanedEstimateReason(estimatedReasons[dimension]);
      evidence.textContent=currentEvidence||(v.status==="not_applicable"?"本公司不进入该模型的计分阶段":missingScore?"尚未取得可核验的该项资料":dimensionScoresAvailable?"服务器未提供有效说明":"请等待最新数据发布");
      if(missingScore&&v.status!=="not_applicable")evidence.classList.add("missing-text");
      summary.append(name,value,evidence);
      ...
      const currentBasis=currentEvidence||(v.status==="not_applicable"?"模型未进入计分，不需要补资料":missingScore?"待补齐原始资料":"—");
      addDefinition(definitions,"公司实际输入",currentBasis,missingScore&&v.status!=="not_applicable"?"missing-text":"");
      addDefinition(definitions,"数据批次","行情日期："+String(r.source_trade_date||marketAsOf||"未公开")+"；该子指标的单独财报报告期未随公开详情提供。");
      addDefinition(definitions,"来源追溯",sourceVersion?"公开详情未附该子指标的单独来源链接；可追溯数据版本："+sourceVersion+"。":"公开详情未附该子指标的单独来源链接或版本号。");
      addDefinition(definitions,"触发阈值",independentGate?"该项必须严格高于7.0分（对应底层账本严格高于70分）。":"该子项按0至10分进入加权总分；类型触发规则："+(method.trigger||"—"));
      addDefinition(definitions,"计算方式",independentGate?"独立门槛判断，不计算可与其他两项抵消的加权贡献。":"类型总分中的该项贡献＝子项分数×"+weight.toFixed(weight%1?1:0)+"%。");
      if(hasScore&&!missingScore&&v.status!=="not_applicable"&&!independentGate)addDefinition(definitions,"总分贡献",(Number(subScores[dimension])*weight/100).toFixed(2)+"分（"+Number(subScores[dimension]).toFixed(1)+" × "+weight.toFixed(weight%1?1:0)+"%）","contribution");
      else if(hasScore&&!missingScore&&v.status!=="not_applicable"&&independentGate)addDefinition(definitions,"门槛结果",Number(subScores[dimension]).toFixed(3)+"分，"+(Number(subScores[dimension])>7?"已严格高于7.0分":"未严格高于7.0分")+"；该项单独判定。","gate-result");
      else if(hasEstimate&&missingScore)addDefinition(definitions,"参考说明","该值只帮助定位缺口，不参与触发或否决。");
      body.append(definitions);item.append(summary,body);dimensions.append(item);
    }
    article.append(title,right,reason,typeMethod,scope);if(evidenceNotes.childElementCount)article.append(evidenceNotes);article.append(dimensions);fragment.append(article);
  }
...
async function load(){try{const manifestResponse=await fetch("/api/manifest",{cache:"no-store"});if(!manifestResponse.ok)throw new Error("清单读取 HTTP "+manifestResponse.status);const m=await manifestResponse.json();generationId=encodeURIComponent(String(m.generation_id||m.provenance?.generation_id||""));marketAsOf=String(m.market_as_of||"");sourceVersion=String(m.provenance?.source_commit||"").slice(0,12);const indexResponse=await fetch("/api/catalogue-index?generation_id="+generationId,{cache:"force-cache"});if(!indexResponse.ok)throw new Error("公司索引读取 HTTP "+indexResponse.status);const c=await indexResponse.json();dimensionScoresAvailable=m.capabilities?.dimension_scores===true&&c.capabilities?.dimension_scores===true;dimensionEstimatesAvailable=m.capabilities?.dimension_score_estimates===true&&c.capabilities?.dimension_score_estimates===true;data=(Array.isArray(c.companies)?c.companies:[]).map(r=>({...r,_search:(String(r.code)+" "+String(r.name)).toLowerCase(),_market:String(r.code).startsWith("6")?"SH":"SZ"}));fillOptions();const s=m.summary||{};renderCoverage(s);const summary=c.summary||{},evidenceGapCompanyCount=Number(summary.evidence_gap_company_count??s.insufficient_company_count??summary.insufficient_company_count??0),decisionRelevantGapCount=Number(summary.decision_relevant_gap_company_count??summary.insufficient_company_count??0);const cards=[["全市场公司",s.company_count||data.length],["至少一种已触发",s.triggered_company_count||0],["待满足附加条件",s.conditional_company_count||0],["其中待确认仓位",summary.action_confirmation_company_count||0],["有客观资料缺口",evidenceGapCompanyCount],["缺口可能改变结论",decisionRelevantGapCount],["七类触发次数",Object.values(s.type_coverage||{}).reduce((n,v)=>n+Number(v.triggered||0),0)]];$("cards").replaceChildren(...cards.map(([label,value])=>{const d=document.createElement("div");d.className="card";const sm=document.createElement("small");sm.textContent=label;const st=document.createElement("strong");st.textContent=Number(value||0).toLocaleString();d.append(sm,st);return d}));$("meta").textContent="数据日期："+(m.market_as_of||"—")+" · 更新时间："+formatBeijing(m.data_timestamp_utc)+" · 来源版本："+(sourceVersion||"—")+" · 量化口径："+METHODOLOGY_LABEL;if(!dimensionScoresAvailable){$("notice").hidden=false;$("notice").textContent="当前服务器数据版本较旧，尚未包含子指标分数，请等待下一次数据发布。"}render()}catch(error){$("meta").textContent="数据读取失败："+String(error?.message||error);$("notice").hidden=false;$("notice").textContent="请稍后重试；如果持续失败，请检查数据发布状态。"}}
...
function declaredAsset(manifest,key){const value=manifest?.[key];const filename=String(value?.filename||""),sha256=String(value?.sha256||"").toLowerCase();const size=Number(value?.size);return{filename,sha256:/^[0-9a-f]{64}$/.test(sha256)?sha256:"",size:Number.isSafeInteger(size)&&size>0?size:null}}
...
async function readCatalogue(env,prefix,manifest){const metadata=declaredAsset(manifest,"catalogue"),object=metadata.filename?await env.DATA_BUCKET.get(prefix+metadata.filename):null;if(!object)throw new Error("目录对象缺失");if(manifest?.company_details&&String(object.customMetadata?.sha256||"").toLowerCase()!==metadata.sha256)throw new Error("目录对象校验标记不一致");const stream=object.body?.pipeThrough(new DecompressionStream("gzip"));if(!stream)throw new Error("目录流不可用");const catalogue=await new Response(stream).json();if(!catalogue||!Array.isArray(catalogue.companies))throw new Error("目录结构无效");return catalogue}
...
async function readCompanyDetail(env,prefix,manifest,generationId,code){
  ...
  const object=await env.DATA_BUCKET.get(prefix+metadata.filename);if(!object)throw new Error("公司详情对象缺失");
  if(object.size!==metadata.size)throw new Error("公司详情对象大小不一致");
  if(String(object.customMetadata?.sha256||"").toLowerCase()!==String(metadata.sha256||"").toLowerCase())throw new Error("公司详情对象校验标记不一致");
  const stream=object.body?.pipeThrough(new DecompressionStream("gzip"));if(!stream)throw new Error("公司详情流不可用");
...
function compactType(value){const decision=value?.decision||{};return{status:String(value?.status||"invalid"),score:value?.score??null,score_lower_bound:decision.score_lower_bound??null,score_upper_bound:decision.score_upper_bound??null,has_missing_dimensions:Array.isArray(decision.missing_dimensions)&&decision.missing_dimensions.length>0}}
function compactCompany(company){const types={};for(const key of Object.keys(company?.types||{}))types[key]=compactType(company.types[key]);return{code:String(company?.code||""),name:String(company?.name||""),industry:String(company?.industry||""),diagnostic_score:company?.diagnostic_score??null,primary_label:String(company?.primary_label||""),types}}
function typeDataGap(value){const decision=value?.decision||{},missing=Array.isArray(decision.missing_dimensions)?decision.missing_dimensions:[],actionRequired=value?.action_required==="position_confirmation",dataMissing=missing.filter(dimension=>!(dimension==="6e"&&actionRequired)),actionOnly=actionRequired&&dataMissing.length===0,declaredIncomplete=value?.applicable===true&&value?.evidence_complete===false,hasGap=dataMissing.length>0||(!actionOnly&&(declaredIncomplete||Boolean(String(value?.evidence_gap||"").trim())));return{has_gap:hasGap,decision_relevant:hasGap&&decision.decision_complete===false,action_required:actionRequired}}
function catalogueGapSummary(companies){let insufficient=0,evidenceGap=0,decisionRelevant=0,boundedGap=0,actionConfirmation=0;for(const company of companies){const entries=Object.entries(company?.types||{}),values=entries.map(([,value])=>value),states=entries.map(([key,value])=>({key,...typeDataGap(value)})),hasInsufficient=values.some(value=>value?.status==="insufficient_evidence"),hasGap=states.some(state=>state.has_gap),hasRelevant=states.some(state=>state.has_gap&&state.decision_relevant),hasAction=states.some(state=>state.action_required||(state.key==="type6"&&company.types?.type6?.status==="conditional"));if(hasInsufficient)insufficient++;if(hasGap)evidenceGap++;if(hasRelevant)decisionRelevant++;if(hasGap&&!hasRelevant)boundedGap++;if(hasAction)actionConfirmation++}return{insufficient_company_count:insufficient,evidence_gap_company_count:evidenceGap,decision_relevant_gap_company_count:decisionRelevant,bounded_gap_company_count:boundedGap,action_confirmation_company_count:actionConfirmation}}
  ...
  async fetch(request,env){
    ...
    try{
      ...
      if(path==="/api/catalogue"){
        const manifest=await generationManifest(env,generation),metadata=declaredAsset(manifest,"catalogue"),object=await env.DATA_BUCKET.get(prefix+metadata.filename);if(!object)return json({error:"目录对象缺失"},503);if(manifest?.company_details&&String(object.customMetadata?.sha256||"").toLowerCase()!==metadata.sha256)return json({error:"目录对象校验标记不一致"},503);const body=object.body?.pipeThrough(new DecompressionStream("gzip"));if(!body)return json({error:"目录流不可用"},503);return new Response(body,{headers:{"content-type":"application/json; charset=utf-8","cache-control":requestedGeneration?"public, max-age=31536000, immutable":"no-store",etag:object.etag||""}});
      }
      if(path==="/api/catalogue-index"){
        const manifest=await generationManifest(env,generation);const build=async()=>{const catalogue=await readCatalogue(env,prefix,manifest);return{generation_id:generation.generation_id,capabilities:catalogue.capabilities||{},summary:{company_count:catalogue.companies.length,...catalogueGapSummary(catalogue.companies)},companies:catalogue.companies.map(compactCompany)}};return requestedGeneration?immutableProjection(request,build):json(await build());
      }
      const companyMatch=path.match(/^\/api\/company\/([036][0-9]{5})$/);
      if(companyMatch){
        const code=companyMatch[1],manifest=await generationManifest(env,generation);const build=async()=>{let company=await readCompanyDetail(env,prefix,manifest,generation.generation_id,code),detailContract="company_detail_v2",capabilities=manifest?.capabilities||{};if(!manifest?.company_details){const catalogue=await readCatalogue(env,prefix,manifest);company=catalogue.companies.find(value=>String(value?.code||"")===code);capabilities=catalogue.capabilities||capabilities;detailContract="legacy_catalogue"}if(!company)return{error:"未找到该公司",code};return{generation_id:generation.generation_id,market_as_of:generation.market_as_of,methodology_version:METHODOLOGY_VERSION,detail_contract:detailContract,capabilities,company}};if(requestedGeneration)return await immutableProjection(request,build);const payload=await build();return headSafeResponse(request,json(payload,payload.error?404:200));
      }
