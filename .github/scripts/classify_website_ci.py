...
# ``atomic_io.py`` is intentionally absent: it is a shared repository utility,
# so changing it must still run the data-integrity gate.
AI_SCREENING_FILES = frozenset(
    {
        ".github/workflows/publish-ai-screening.yml",
        "cloudflare/quant-dashboard/ai_screening_seed.json",
        "tools/ai_company_research_provenance.py",
        "tools/ai_quantitative_facts.py",
        "tools/ai_source_urls.py",
        ...
        "tools/ai_screening_reasonix_prompt.md",
        ...
        "tools/codex_web_events.py",
        "tools/sanitize_codex_luna_reviews.py",
        "tools/apply_ai_screening_human_review.py",
        "tools/apply_deep_review_ai_screening.py",
        "tools/assemble_ai_screening_reviews.py",
        "tools/build_local_ai_review.py",
        "tools/deep_review_ai_screening.py",
        "tools/enrich_ai_screening_input.py",
        "tools/enrich_ai_screening_quantitative_facts.py",
        "tools/merge_ai_screening_reviews.py",
        "tools/prepare_ai_screening_overlay.py",
        "tools/render_ai_screening_preview.py",
        "tools/run_ai_screening_batch.py",
        "tools/run_ai_screening_reasonix.py",
        "tools/shard_ai_screening_candidates.py",
        "tools/opencode-screening/opencode.json",
        "tools/ai_screening_human_review_2026-08-21.json",
        "tools/ai_screening_overrides_2026-08-20.json",
        ...
        "tests/test_ai_quantitative_facts.py",
        "tests/test_ai_screening_websearch_events.py",
        "tests/test_local_ai_review.py",
        "tests/test_shard_ai_screening_candidates.py",
    }
)
WEB_FILES = frozenset(
    {
...
def classify_paths(paths: Iterable[str]) -> WebsiteGates:
    ...
    for path in normalized:
        if path in AI_SCREENING_FILES:
            # The dedicated AI lane owns this compact contract suite. A
            # seed-only change additionally starts the publication workflow,
            # but must not repeat the general Worker or data-integrity suites.
            ai = True
            continue
        if path in RUN_ALL_FILES:
            web = True
            data = True
            ai = True
            ...
            data = True
    return WebsiteGates(web=web, data=data, ai=ai)


def _write_outputs(gates: WebsiteGates, output_path: Path | None) -> None:
    lines = (
        f"web={str(gates.web).lower()}",
        f"data={str(gates.data).lower()}",
        f"ai={str(gates.ai).lower()}",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--all", action="store_true", help="run all gates (manual or unbounded revision)")
    parser.add_argument("--github-output", type=Path)
    args = parser.parse_args(argv)

    gates = WebsiteGates(web=True, data=True, ai=True) if args.all else classify_paths(sys.stdin)
