...
- Type7 的“长期质量与回报”“产业质量与估值”“商业质量与安全边际”三项百分制评分必须分别 `>70`，不能用平均分相互补偿；全部必需子项和其他前置核验必须通过，安全边际 `<8/20` 时直接否决。十年回报使用后复权周线总回报代理，并量化当前市值/十年后预测利润；估值位置使用五年日度 PE/PB，且每个基准必须独立具备至少500个有效历史观测才参与分位和中位数。五年回报回归中 PE 匹配盈利增长，PB 匹配扣除股本稀释后的每股归母权益增长，不能混用。历史起点晚于目标窗口62天、外部证据晚于行情快照日、缺三家不同发布方且URL不同的外部报告或任一前置证据时均显示证据不足，不用零分伪装失败。
